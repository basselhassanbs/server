                    ...
                    @if (Auth::user()->office_id == 4)
                        ...
                        <div class="dropdown-menu">
                            @foreach ($documentstypes as $value)
                                <a class="dropdown-item" href="/requests/{{ $value->type }}/create">{{ $value->name }}</a>
                            @endforeach
                        </div>
                      </div>
                    @endif
                        ...
                        <table class="table card-table">
                            ...
                            <tbody>
                                @foreach ($documents as $document)
                                    <tr>
                                        <td><a href="/requests/{{ $document->type }}/{{ $document->id }}">{{ $document->description['student_name'] }}</a></td>
                                        <td>{{ $document->name }}</td>
                                        <td>{{ $document->created_at->toDateString() }}</td>
                                        <td>
                                            <a href="/requests/{{ $document->type }}/{{ $document->id }}/edit" class="btn btn-primary btn-sm">تعديل</a>
                                            ...
                                            <form action="/requests/{{ $document->id }}/move" method="POST" id="move-{{ $document->id }}" style="display: none">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                            {{-- @if (auth()->user()->office_id === 4) --}}
                                                ...
                                                <form action="/requests/{{ $document->id }}" method="POST" id="delete-{{ $document->id }}" style="display: none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            {{-- @endif  --}}
